...
class LimeGraphicsObject { 
    ...
    public var objects:Array<LimeObject> = [];
    ...
    public function worldAngle(obj:LimeObject):Float
    {
        if(obj.parent == null)
        {
            return obj.angle;
        ...
        {
            return worldAngle(obj.parent) + obj.angle;
        }
    }

    public function worldScale(obj:LimeObject):Float
    {
        if(obj.parent == null)
        {
            return obj.scale;
        ...
        {
            return worldScale(obj.parent) * obj.scale;
        }
    }

    public function worldPosition(obj:LimeObject):Vec2
    {
        if(obj.parent == null)
        {
            return obj.position;
        ...
        {
            var worldPos = obj.position.copy();
            return worldPos.rotate(worldAngle(obj.parent)).muleq(worldScale(obj.parent)).addeq(worldPosition(obj.parent));
        }
    }
    ...
    public static function compareGraphic(obj1:LimeObject,obj2:LimeObject)
    {
    ...
    public function render(obj:LimeObject,program:GLProgram,customRenderPreFunc:LimeObject->Void,customRenderPostFunc:LimeObject->Void)
    {
        vertexAttribute = GL.getAttribLocation (program, "aPosition");
        textureAttribute = GL.getAttribLocation (program, "aTexCoord");
        ...
        GL.uniform2f(scaleUniformLocation,flip*worldScale(obj) * drawWidth,worldScale(obj) * drawHeight);
        // GL.uniform1f(rotationUniformLocation,worldAngle(obj));

        customRenderPreFunc(obj);
        var mvMatrix = new lime.math.Matrix4();
        // trace(width,height);
        mvMatrix.appendScale(worldScale(obj),worldScale(obj),1);
        // mvMatrix.appendScale(width*worldScale(obj),height*worldScale(obj),1);
        if(worldAngle(obj) != 0)
        {
            mvMatrix.appendRotation(worldAngle(obj)/Math.PI*180,lime.math.Vector4.Z_AXIS);
        }
        mvMatrix.appendTranslation(worldPosition(obj).x,worldPosition(obj).y,obj.z);
