...
class LimeLayer{
    ...
    public var layer:LimeGraphics;

    var renderbuffer:GLRenderbuffer;

    public function new(l:LimeGraphics)
    {
        layer = l;
    }
    ...
    public function render(window:lime.ui.Window,limeGraphics:LimeGraphics)
    {
        bind();
        ...
        var matrix = lime.math.Matrix4.createOrtho (-vWidth/2/limeGraphics.cameraScale + limeGraphics.camera.x + sceneOffset.x, vWidth/2/limeGraphics.cameraScale + limeGraphics.camera.x + sceneOffset.x, -vHeight/2/limeGraphics.cameraScale + limeGraphics.camera.y + sceneOffset.y, vHeight/2/limeGraphics.cameraScale + limeGraphics.camera.y + sceneOffset.y, -2000, 2000);
        GL.uniformMatrix4fv (projectionMatrixUniform, false, matrix);


        LimeGraphicsObject.bindGeneral();
        var lastObj:LimeGraphicsObject;
        for(graphicObj in limeGraphics.graphicIdMap)
        {
            graphicObj.bind();
